...
Finally, on the turtlebot folder, run ```./sync_devices.sh``` to synchronize all the devices times (make sure to have chrony on your laptop ```sudo apt update && sudo apt install chrony```).
