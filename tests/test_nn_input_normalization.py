...
def forward_fn(x, update_stats):
    bn = hk.ExponentialMovingAverage(decay=0.99, zero_debias=False)
...
norm_params, norm_state = model.init(random.PRNGKey(0), vnet_input[0], update_stats=True)
all_normalized_inputs = jnp.zeros((n_steps,)+vnet_input.shape)
all_inputs = jnp.zeros((n_steps,)+vnet_input.shape)
@jit
def apply_model(norm_params, norm_state, vnet_input):
    return vmap(model.apply, in_axes=(None,None,None,0,None))(
        norm_params,
        norm_state,
        None,
        vnet_input,
...
for step in range(n_steps):
    ...
    norm_vnet_input, batch_norm_state = apply_model(
        norm_params, 
        norm_state, 
        vnet_input, 
    )
    norm_state = tree_map(lambda x: x[-1], batch_norm_state)
    # Save data
