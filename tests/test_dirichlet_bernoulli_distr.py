...
samples = distribution.batch_sample(alpha, p, keys)

# Compute entropy of distribution
entropy = distribution.entropy(alpha, p)
print("Entropy: {:.2f}".format(entropy))

# Compute mean action and its probability
mean_v, mean_w = distribution.mean(alpha, p)
print("Mean action: [{:.2f}, {:.2f}]".format(mean_v, mean_w), " - PDF value: ", "{:.2f}".format(distribution.p(alpha, p, jnp.array([mean_v, mean_w]))))
# Compute probability of several actions
print("PDF value of action [0, 0]: ", "{:.2f}".format(distribution.p(alpha, p, jnp.array([0, 0]))))
print("PDF value of action [{:.2f}, 0]: ".format(vmax), "{:.2f}".format(distribution.p(alpha, p, jnp.array([vmax, 0]))))
print("PDF value of action [{:.2f}, 0]: ".format(-vmax), "{:.2f}".format(distribution.p(alpha, p, jnp.array([-vmax, 0]))))
print("PDF value of action [{:.2f}, 0]: ".format(2*vmax), "{:.2f}".format(distribution.p(alpha, p, jnp.array([2*vmax, 0]))))
print("PDF value of action [0, 1]: ", "{:.2f}".format(distribution.p(alpha, p, jnp.array([0, 1]))))
print("PDF value of action [0, -1]: ", "{:.2f}".format(distribution.p(alpha, p, jnp.array([0, -1]))))
print("PDF value of action [{:.2f}, 1]: ".format(vmax/3), "{:.2f}".format(distribution.p(alpha, p, jnp.array([vmax/3, 1]))))
print("PDF value of action [{:.2f}, -1]: ".format(vmax/3), "{:.2f}".format(distribution.p(alpha, p, jnp.array([vmax/3, -1]))))
print("PDF value of action [0, {:.2f}]: ".format(2*vmax/wheels_distance), "{:.2f}".format(distribution.p(alpha, p, jnp.array([0, 2*vmax/wheels_distance]))))
print("PDF value of action [0, {:.2f}]: ".format(-2*vmax/wheels_distance), "{:.2f}".format(distribution.p(alpha, p, jnp.array([0, -2*vmax/wheels_distance]))))
...
pdf_values = distribution.batch_p(alpha, p, more_samples)
fig = plt.figure()
