...
env_params = {
    ...
    'humans_policy': 'hsfm',
    'reward_function': Reward2(discomfort_distance=ds),
    'kinematics': 'unicycle',
}
env = SocialNav(**env_params)

# Initialize robot policy and vnet params
policy = SARLPPO(env.reward_function, distribution='gaussian', dt=env_params['robot_dt'], kinematics='unicycle')
...
def _fori_body(i:int, for_val:tuple):
    ...
    def _while_body(while_val:tuple):
        ...
        action, policy_key, _, _, _, _ = policy.act(policy_key, obs, info, sarl_ppo_params, sample=False, update_stats=False)
...
ax.set(
    xlabel='Reward type', 
    ylabel='Return', 
    title=f'Cumulative reward for each reward type - {n_episodes} trials - {n_humans} humans - robot policy: sarl_ppo\nDs: {ds}, Wp: {wp}, Wt: {wt}, Wr: {wr}, Wbound: {w_bound}',  
...
ax.boxplot(np.transpose(np.array([metrics['returns0'], metrics['returns1'], metrics['returns2'], metrics['returns3']])), widths=0.4, patch_artist=True, 
            ...
            showmeans=True, 
            zorder=1)
figure.savefig(os.path.join(os.path.dirname(__file__),f"sarl_ppo_return_contributions_{n_humans}humans.png"), format='png')