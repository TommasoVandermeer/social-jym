...
for i in range(n_episodes):
    ...
    for j in range(3):
        ...
        # Plot critic heatmap
        vmin = min(jnp.min(value_map_single_gamma), jnp.min(value_map_multi_gamma))
        vmax = max(jnp.max(value_map_single_gamma), jnp.max(value_map_multi_gamma))
        ...
        im1 = axs[0].pcolormesh(X, Y, value_map_single_gamma, vmin=vmin, vmax=vmax, cmap='viridis', shading='nearest')
        axs[0].set_title('Single-Discount')
        im2 = axs[1].pcolormesh(X, Y, value_map_multi_gamma, vmin=vmin, vmax=vmax, cmap='viridis', shading='nearest')
        ...
        fig.suptitle(f"Critic Value Landscape - Episode {i+1}, Step {j*30 + 1}")
        plt.show()
        # Step the environment 30 times
        for _ in range(30):
