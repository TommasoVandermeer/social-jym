...
lidar_configurations = [ # (num_rays, angular_range, n_stack) #
    ...
    (100, jnp.pi / 2, 5), # Heavily reduced angular range 
    (500, jnp.pi * 2, 5), # Heavily augmented resolution
    (20, jnp.pi * 2, 5), # Heavily reduced resolution
]
# Tests
...
if not os.path.exists(os.path.join(os.path.dirname(__file__),"jessi_lidar_agnostic_tests.pkl")):
    ...
    for n, conf in enumerate(lidar_configurations):
        if not os.path.exists(os.path.join(os.path.dirname(__file__),f"jessi_multitask_lidar_agnostic_{conf[0]}R_{(conf[1]/jnp.pi)*180:.0f}AR_{conf[2]}NS_tests.pkl")):
            ...
            # Execute tests
            policy = JESSI(
                lidar_num_rays=conf[0],
                lidar_angular_range=conf[1],
                lidar_max_dist=10.0,
                n_stack=conf[2],
    ...
    with open(os.path.join(os.path.dirname(__file__),"jessi_lidar_agnostic_tests.pkl"), 'rb') as f:
