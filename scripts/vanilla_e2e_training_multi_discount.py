...
bounded_network_name = 'bounded_vanilla_e2e_single_gamma_rl_out.pkl'
unbounded_network_name_multi_gamma = 'vanilla_e2e_multi_gamma_rl_out.pkl'
bounded_network_name_multi_gamma = 'bounded_vanilla_e2e_multi_gamma_rl_out.pkl'
...
### RL UNBOUNDED POLICY NETWORK SINGLE GAMMA
if not os.path.exists(os.path.join(os.path.dirname(__file__), unbounded_network_name)):
    ...
    policy = VanillaE2E(
        ...
        critic_heads=len(reward_function.unique_gammas),
    )
    # Initialize weights
    il_network_params = policy.init_nn(random.PRNGKey(seed))
    ...
    print(f"UNBOUNDED VANILLA-E2E SINGLE-GAMMA training already done and saved... Remove '{unbounded_network_name}' to retrain.")


### RL BOUNDED POLICY NETWORK SINGLE GAMMA
if not os.path.exists(os.path.join(os.path.dirname(__file__), bounded_network_name)):
    ...
    print(f"BOUNDED VANILLA-E2E SINGLE-GAMMA training already done and saved... Remove '{bounded_network_name}' to retrain.")

### RL UNBOUNDED POLICY NETWORK MULTI GAMMA
if not os.path.exists(os.path.join(os.path.dirname(__file__), unbounded_network_name_multi_gamma)):
    ...
    rl_out = vanilla_e2e_rl_rollout(**rl_rollout_params)
    # Save RL rollout output
    with open(os.path.join(os.path.dirname(__file__),unbounded_network_name_multi_gamma), 'wb') as f:
        pickle.dump(rl_out, f)
    final_params, _, metrics = rl_out
    processed_metrics = {}
    for key, value in metrics.items():
    ...
    figure.savefig(os.path.join(os.path.dirname(__file__),unbounded_network_name_multi_gamma.replace('.pkl', '.eps')), format='eps')
else:
    print(f"UNBOUNDED VANILLA-E2E training already done and saved... Remove '{unbounded_network_name_multi_gamma}' to retrain.")


### RL BOUNDED POLICY NETWORK SINGLE GAMMA
if not os.path.exists(os.path.join(os.path.dirname(__file__), bounded_network_name_multi_gamma)):
    ...
    rl_out = vanilla_e2e_rl_rollout(**rl_rollout_params)
    # Save RL rollout output
    with open(os.path.join(os.path.dirname(__file__),bounded_network_name_multi_gamma), 'wb') as f:
        pickle.dump(rl_out, f)
    final_params, _, metrics = rl_out
    processed_metrics = {}
    for key, value in metrics.items():
    ...
    figure.savefig(os.path.join(os.path.dirname(__file__),bounded_network_name_multi_gamma.replace('.pkl', '.eps')), format='eps')
else:
    print(f"BOUNDED VANILLA-E2E MULTI-GAMMA training already done and saved... Remove '{bounded_network_name_multi_gamma}' to retrain.")