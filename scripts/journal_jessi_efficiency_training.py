...
policy_nn_name = 'pre_controller_network'
multitask_network_name = 'jessi_multitask_rl_out'
modular_network_name = 'jessi_modular_rl_out'
policy_network_name = 'jessi_policy_rl_out'
...
for embeddings_dim in network_embeddings_dims:
    ...
    if not os.path.exists(os.path.join(os.path.dirname(__file__),  f"{perception_nn_name}_{embeddings_dim}.pkl")):
        ...
        with open(os.path.join(os.path.dirname(__file__), f"{perception_nn_name}_{embeddings_dim}.pkl"), 'wb') as f:
        ...
        ax.set_ylabel("Loss")
        fig.legend()
        fig.savefig(os.path.join(os.path.dirname(__file__), f"{perception_nn_name}_{embeddings_dim}.eps"), format='eps')
        del train_dataset
        del val_dataset
        del test_dataset
    else:
        # Load trained parameters
        with open(os.path.join(os.path.dirname(__file__), f"{perception_nn_name}_{embeddings_dim}.pkl"), 'rb') as f:
            encoder_params = pickle.load(f)

    ### PRE-TRAIN POLICY NETWORK (IMITATION LEARNING)
    if not os.path.exists(os.path.join(os.path.dirname(__file__), f"{policy_nn_name}_{embeddings_dim}.pkl")):
        ...
        # Save trained parameters
        with open(os.path.join(os.path.dirname(__file__), f"{policy_nn_name}_{embeddings_dim}.pkl"), 'wb') as f:
        ...
        fig.savefig(os.path.join(os.path.dirname(__file__), f"{policy_nn_name}_{embeddings_dim}.eps"), format='eps')
    else:
        # Load trained parameters
        with open(os.path.join(os.path.dirname(__file__), f"{policy_nn_name}_{embeddings_dim}.pkl"), 'rb') as f:
            actor_critic_params = pickle.load(f)

    ### JESSI-MULTITASK: MULTI-TASK REINFORCEMENT LEARNING
    if not os.path.exists(os.path.join(os.path.dirname(__file__), f"{multitask_network_name}_{embeddings_dim}.pkl")):
        ...
        with open(os.path.join(os.path.dirname(__file__), f"{multitask_network_name}_{embeddings_dim}.pkl"), 'wb') as f:
        ...
        figure.savefig(os.path.join(os.path.dirname(__file__),f"{multitask_network_name}_{embeddings_dim}.eps"), format='eps')
    else:
        print(f"JESSI-MULTITASK RL training already done and saved... Remove '{multitask_network_name}_{embeddings_dim}' to retrain.")