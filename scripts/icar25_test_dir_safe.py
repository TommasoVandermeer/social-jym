...
figure2, ax2 = plt.subplots(4, 3, figsize=(15, 20))
figure.subplots_adjust(hspace=0.4, wspace=0.3, bottom=0.05, top=0.95, left=0.08, right=0.82)
figure2.subplots_adjust(hspace=0.4, wspace=0.3, bottom=0.05, top=0.95, left=0.08, right=0.82)
for m, metric in enumerate(metrics_to_plot):
    ...
    ax2[i,j].grid(zorder=0)
    ax2[i,j].set_xticks(jnp.arange(len(tests_n_humans)))
    ax2[i,j].set_xticklabels(tests_n_humans)
    for s, scenario in enumerate(tests_scenarios):
        if metric in ['successes', 'collisions', 'timeouts']:
            ...
            y_dwa_data = jnp.nanmean(all_metrics_dwa[metric][s, :, :], axis=1) / n_trials
            ax[i, j].set_ylim(-0.05, 1.05)
            ax2[i, j].set_ylim(-0.05, 1.05)
        else:
            y_data = jnp.nanmean(all_metrics[metric][s, :, :, :], axis=(1,2))
            y_dwa_data = jnp.nanmean(all_metrics_dwa[metric][s, :, :, :], axis=(1,2))
        ax[i, j].plot(jnp.arange(len(tests_n_humans)), y_data, label=scenarios[scenario]["label"], color=colors[s], linewidth=2.5)
        ax2[i, j].plot(jnp.arange(len(tests_n_humans)), y_dwa_data, label=scenarios[scenario]["label"], color=colors[s], linewidth=2.5)
h, l = ax[0,0].get_legend_handles_labels()
figure.legend(h, l, loc='center right', title='Test scenario')
figure2.legend(h, l, loc='center right', title='Test scenario')
figure.savefig(os.path.join(os.path.dirname(__file__), "icar25_soarld_test_results_1.eps"), format='eps')
figure2.savefig(os.path.join(os.path.dirname(__file__), "icar25_dwa_test_results_1.eps"), format='eps')
...
for m, metric in enumerate(metrics_to_plot):
    ...
    ax2[i,j].set(
        ...
        title=metrics[metric]['label'],)
    ax2[i,j].grid(zorder=0)
    ax2[i,j].set_xticks(jnp.arange(len(tests_n_obstacles)))
    ax2[i,j].set_xticklabels(tests_n_obstacles)
    for s, scenario in enumerate(tests_scenarios):
        if metric in ['successes', 'collisions', 'timeouts']:
            y_data = jnp.nanmean(all_metrics[metric][s, :, :], axis=0) / n_trials
            y_dwa_data = jnp.nanmean(all_metrics_dwa[metric][s, :, :], axis=0) / n_trials
            ax[i, j].set_ylim(-0.05, 1.05)
            ax2[i, j].set_ylim(-0.05, 1.05)
        else:
            y_data = jnp.nanmean(all_metrics[metric][s, :, :, :], axis=(0,2))
            y_dwa_data = jnp.nanmean(all_metrics_dwa[metric][s, :, :, :], axis=(0,2))
        ax[i, j].plot(jnp.arange(len(tests_n_obstacles)), y_data, label=scenarios[scenario]["label"], color=colors[s], linewidth=2)
        ax2[i, j].plot(jnp.arange(len(tests_n_obstacles)), y_dwa_data, label=scenarios[scenario]["label"], color=colors[s], linewidth=2)
h, l = ax[0,0].get_legend_handles_labels()
figure.legend(h, l, loc='center right', title='Test scenario')
figure2.legend(h, l, loc='center right', title='Test scenario')
figure.savefig(os.path.join(os.path.dirname(__file__), "icar25_soarld_test_results_2.eps"), format='eps')
figure2.savefig(os.path.join(os.path.dirname(__file__), "icar25_dwa_test_results_2.eps"), format='eps')
