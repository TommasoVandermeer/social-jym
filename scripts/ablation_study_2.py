...
for i, n_humans in enumerate(test_n_humans):
    ...
    ax[3, i].boxplot(
        ...
        showfliers=False,
        showmeans=True)
    ax[0, i].set_xticklabels(scenarios_labels + ["All"])
    ax[1, i].set_xticklabels(scenarios_labels + ["All"])
    ax[2, i].set_xticklabels(scenarios_labels + ["All"])
    ax[3, i].set_xticklabels(scenarios_labels + ["All"])
# Save figure
figure.savefig(os.path.join(os.path.dirname(__file__),f"metrics_boxplots_after_rl_full_and_base_all_scenarios_ablation_study.png"), format='png')

for s_idx, scenario in enumerate(test_scenarios):
    # Plot boxplot of each metric after RL for base reward and reward with all contributions
    figure, ax = plt.subplots(math.ceil((len(all_metrics_after_rl)-4)/3), 3, figsize=(10,10))
    figure.suptitle(f"Metrics after RL training for each test for base and full rewards - {scenario} - {n_test_trials} trials")
    ...
    for key, values in all_metrics_after_rl.items():
        if key == "successes" or key == "collisions" or key == "timeouts":
            ...
            ax[i,j].grid()
            # Base reward
            data = pd.DataFrame(np.transpose(values[0,s_idx]), columns=test_n_humans)
            data = data.dropna()
            ax[i,j].boxplot(data, widths=0.4, patch_artist=True, 
                boxprops=dict(facecolor='lightblue', edgecolor='lightblue', alpha=0.7),
                tick_labels=test_n_humans,
                ...
                zorder=1)
            # Full reward
            data = pd.DataFrame(np.transpose(values[-1,s_idx]), columns=test_n_humans)
            data = data.dropna()
            ax[i,j].boxplot(data, widths=0.3, patch_artist=True, 
                    boxprops=dict(facecolor="lightcoral", edgecolor="lightcoral", alpha=0.4),
                    tick_labels=test_n_humans,
                    ...
                    zorder=2)
            idx += 1
    figure.savefig(os.path.join(os.path.dirname(__file__),f"metrics_boxplots_after_rl_full_and_base_{scenario}_ablation_study.png"), format='png')

    # Plot curves of each metric after RL for each reward
    figure, ax = plt.subplots(math.ceil((len(all_metrics_after_rl)-4)/3), 3, figsize=(10,10))
    figure.suptitle(f"Metrics after RL training for each test - {scenario} - {n_test_trials} trials")
    figure.subplots_adjust(hspace=0.5, wspace=0.5, bottom=0.05, top=0.90, left=0.1, right=0.87)
    idx = 0
    for key, values in all_metrics_after_rl.items():
        if key == "successes" or key == "collisions" or key == "timeouts":
            ...
            for reward in range(len(values)):
                ax[i,j].plot(test_n_humans, np.nanmean(values[reward,s_idx], axis=1), color=list(mcolors.TABLEAU_COLORS.values())[reward])
            idx += 1
    figure.legend(["Reward {}".format(i) for i in range(len(all_metrics_after_rl["times_to_goal"]))], loc="center right")
    figure.savefig(os.path.join(os.path.dirname(__file__),f"metrics_after_rl_{scenario}_ablation_study.png"), format='png')
    ...
    for test, n_humans in enumerate(test_n_humans):
        figure, ax = plt.subplots(math.ceil((len(all_metrics_after_rl)-4)/3), 3, figsize=(10,10))
        figure.suptitle(f"Metrics after RL training for each test for base and full rewards - {scenario} - {n_humans} humans - {n_test_trials} trials")
        figure.subplots_adjust(hspace=0.5, wspace=0.5, bottom=0.05, top=0.90, left=0.1, right=0.87)
        idx = 0
        for key, values in all_metrics_after_rl.items():
            if key == "successes" or key == "collisions" or key == "timeouts":
                ...
                ax[i,j].grid()
                # All rewards
                data = pd.DataFrame(np.transpose(values[:,s_idx,test]), columns=["Reward {}".format(i) for i in range(len(values))])
                data = data.dropna()
        ...
        figure.legend(handles=legend_elements, loc="center right")
        figure.savefig(os.path.join(os.path.dirname(__file__),f"metrics_boxplots_after_rl_{n_humans}humans_{scenario}_ablation_study.png"), format='png')
