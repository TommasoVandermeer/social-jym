...
def deep_a2c_rl_rollout_2(
        ...
        def _update_for_loop_body(upd_idx:int, val:tuple):
                # Retrieve data from the tuple
                policy_keys, reset_keys, states, obses, infos, inputs, outcomes, actor_params, critic_params, act_opt_state, cri_opt_state, buffer_state, current_buffer_size, aux_data = val
                # Compute sigma for the episode
                sigma = sigma_decay_fn(sigma_start, sigma_end, upd_idx, sigma_decay)
                ## Generate experience
                @jit
                def _step_fori_body(step:int, val:tuple):
                        # Retrieve data from the tuple
                        actor_params, critic_params, states, obses, infos, inputs, init_outcomes, policy_keys, reset_keys, sigma, episode_count, batch_inputs, batch_values, batch_actions, batch_rewards, batch_dones, successes, failures, timeouts, returns = val
                ...
                val_init = (actor_params, critic_params, states, obses, infos, initial_inputs, init_outcomes, policy_keys, reset_keys, sigma, 0, batch_inputs, batch_values, batch_actions, batch_rewards, batch_dones, 0, 0, 0, jnp.zeros(n_parallel_envs,))
                actor_params, critic_params, states, obses, infos, inputs, outcomes, policy_keys, reset_keys, sigma, episode_count, batch_inputs, batch_values, batch_actions, batch_rewards, batch_dones, succ_count, fail_count, timeo_count, cum_rewards = lax.fori_loop(
                        0,
                        n_steps, 
                        _step_fori_body, 
                        val_init
                ...
                current_buffer_size = 0
                # Save data
                aux_data["actor_losses"] = aux_data["actor_losses"].at[upd_idx].set(actor_loss)
                aux_data["critic_losses"] = aux_data["critic_losses"].at[upd_idx].set(critic_loss)
                aux_data["entropy_losses"] = aux_data["entropy_losses"].at[upd_idx].set(entropy_loss)
                # DEBUG: print the mean loss for this episode and the return
                lax.cond(
                        (debugging) & (upd_idx % debugging_interval == 0) & (upd_idx != 0),   
                        lambda _: debug.print(
                ...
                return policy_keys, reset_keys, states, obses, infos, inputs, outcomes, actor_params, critic_params, act_opt_state, cri_opt_state, buffer_state, current_buffer_size, aux_data
