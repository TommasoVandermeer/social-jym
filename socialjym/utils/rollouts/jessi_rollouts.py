        ...
        def _update_for_loop_body(upd_idx:int, val:tuple):
                ...
                lax.cond(
                        ...
                        lambda _: debug.print(
                                ...
                                llv=jnp.nanmean(jnp.where((jnp.arange(len(aux_data["loss_stds"]))[:,None,None] > upd_idx-debugging_interval) & (jnp.arange(len(aux_data["loss_stds"]))[:,None,None] <= upd_idx), aux_data["loss_stds"], jnp.array([jnp.nan,jnp.nan,jnp.nan])), axis=0),
