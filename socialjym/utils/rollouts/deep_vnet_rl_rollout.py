...
def deep_vnet_rl_rollout(
        ...
        def _fori_body(i: int, val:tuple):
                ...
                def _while_body(val:tuple):
                        ...
                        state, obs, info, done, policy_key, buffer_state, current_buffer_size = val
                        # Step
                        ...
                        return (state, obs, info, done, policy_key, buffer_state, current_buffer_size)
                # Retrieve data from the tuple
                model_params, optimizer_state, buffer_state, current_buffer_size, policy_key, buffer_key, reset_key, losses = val
                # Reset the environment
                state, reset_key, obs, info = env.reset(reset_key)
                # Episode loop
                val_init = (state, obs, info, False, policy_key, buffer_state, current_buffer_size)
                _, _, _, _, policy_key, buffer_state, current_buffer_size = lax.while_loop(lambda x: x[3] == False, _while_body, val_init)           
                ...
                val = (model_params, optimizer_state, buffer_state, current_buffer_size, policy_key, buffer_key, reset_key, losses)
                return val
        ...
        losses = jnp.zeros([train_episodes], dtype=jnp.float32)
        # Initialize the model parameters
        model_params = model.init(params_key, jnp.zeros(vnet_input_shape))
        optimizer_state = optimizer.init(model_params)
        # Create initial values for training loop
        val_init = (model_params, optimizer_state, buffer_state, 0, policy_key, buffer_key, reset_key, losses)
        # Execute the training loop
        vals = lax.fori_loop(0, train_episodes, _fori_body, val_init)

        output_dict = {}
        keys = ["model_params", "optimizer_state", "buffer_state", "current_buffer_size", "policy_key", "buffer_key", "reset_key", "losses"]
