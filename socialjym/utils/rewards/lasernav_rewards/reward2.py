...
from socialjym.envs.base_env import ROBOT_KINEMATICS, HUMAN_POLICIES, wrap_angle
...
class Reward2(BaseReward):
    def __init__(
        ...
        accountability_for_human_proximity_reward: bool=True,
        goal_reward: float=1., 
        ...
        assert (accountability_fov > 0) and (accountability_fov < 360), "accountability fov must be in [0,360°]"
        assert accountability_danger_dist > 0, "accoutability danger distance must be greater than 0"
        assert accountability_fast_penalty_coeff > 0, "accountability fast penalty coefficient must be greater than 0"
        assert accountability_stop_reward_coeff > 0, "accountability stop reward coefficient must be greater than 0"
        # Define reward type
        ...
        self.accountability_for_human_proximity_reward = accountability_for_human_proximity_reward
        self.binary_reward = jnp.array(
            [
                ...
                accountability_for_human_proximity_reward,
            ], 
            dtype=int
        )
    ...
    def __call__(
        ...
        if self.high_rotation_penalty_reward:
            ...
            rotation_reward = 0.
        # Penalty for proximity to human accountability
        if self.accountability_for_human_proximity_reward:
            ...
            accountability_reward = jnp.where(
                in_zone,
                jnp.where(is_fast, fast_penalty, stop_reward),
                0.0
            )
        else:
            accountability_reward = 0.
        reward = goal_reward + collision_human_reward + collision_obstacle_reward + discomfort_reward + progress_reward + rotation_reward + accountability_reward
        if self.multi_gamma:
