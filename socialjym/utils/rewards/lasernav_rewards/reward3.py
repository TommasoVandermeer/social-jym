...
class Reward3(BaseReward):
    def __init__(
        ...
        assert discomfort_k_front > 0, "discomfort_k_front must be positive"
        assert discomfort_alpha > 0, "discomfort_alpha must be positive"
        assert discomfort_weight > 0, "discomfort_weight must be positive"
        ...
        self.type = f"lasernav_reward3_{self.decimal_reward}"
        ...
        self.discomfort_k_front = discomfort_k_front
        self.discomfort_alpha = discomfort_alpha
    ...
    def __call__(
        ...
        if self.discomfort_distance_penalty_reward:
            delta_pos = next_robot_pos - next_humans_pos # Shape: (N_humans, 2)
            distances = jnp.linalg.norm(delta_pos, axis=1)
            human_speeds = jnp.linalg.norm(humans_velocities, axis=1)
            safe_distances = distances + 1e-8
            safe_speeds = human_speeds + 1e-8
            dot_products = jnp.sum(delta_pos * humans_velocities, axis=1)
            cos_angles = dot_products / (safe_distances * safe_speeds)
            dynamic_margins = self.discomfort_distance + (self.discomfort_k_front * human_speeds * jnp.maximum(0.0, cos_angles))
            bound_distances = distances - robot_radius - humans_radiuses
            violations = dynamic_margins - bound_distances
            penetrations = jnp.maximum(0.0, violations)
            discomfort_penalties = self.discomfort_weight * (jnp.exp(self.discomfort_alpha * penetrations) - 1.0)
            discomfort_reward = lax.cond(
                failure,
                lambda: 0.,
                lambda: - dt * jnp.sum(discomfort_penalties)
            )
        else:
            discomfort_reward = 0.
