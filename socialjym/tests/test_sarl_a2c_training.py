...
from socialjym.utils.aux_functions import test_k_trials, animate_trajectory
from socialjym.utils.rewards.socialnav_rewards.reward1 import Reward1
from socialjym.utils.rollouts.deep_a2c_rollouts import deep_a2c_il_rollout
...
training_hyperparams = {
    ...
    'rl_training_episodes': 30_000, # Holonomic kinematics 10.000 episodes, Unicycle kinematics 30.000 episodes
...
replay_buffer = BaseA2CBuffer(training_hyperparams['buffer_size'], training_hyperparams['batch_size'])
...
ax[0].set(
    xlabel='Epoch', 
    ylabel='Loss', 
    title='Actor Loss during IL training for each seed'
)
ax[0].plot(
...
ax[1].set(
    xlabel='Epoch', 
    ylabel='Loss', 
    title='Critic Loss during IL training for each seed'
)
ax[1].plot(
...
while(input("Do you want to watch some trials after IL? (y/n): ") == 'y'):
    ...
    while outcome["nothing"]:
        action, policy_key, _, sampled_action, distrs = policy.act(policy_key, obs, info, il_actor_params, True)
