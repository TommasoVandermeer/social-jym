...
# Initial conditions
humans_state = jnp.array([[7.,0.,0.,0.,jnp.pi,0.],
...
keys = jax.random.split(jax.random.PRNGKey(seed), n_humans)
leg_dists = vmap(lambda k: jax.random.uniform(k, minval=0.3, maxval=0.7))(keys) * humans_parameters[:,0]
for i in range(n_humans):
    # Goal: (gx, gy)
    humans_goal[i,0] = -room_half_length
    humans_goal[i,1] = 0.
    # Legs state: (x1, y1, phase1, x2, y2, phase2)
    humans_leg_state[i,0] = humans_state[i,0] + leg_dists[i] * jnp.cos(humans_state[i,4] + jnp.pi/2) + (step_length / 2) * jnp.cos(humans_state[i,4])
    humans_leg_state[i,1] = humans_state[i,1] + leg_dists[i] * jnp.sin(humans_state[i,4] + jnp.pi/2) + (step_length / 2) * jnp.sin(humans_state[i,4])
    humans_leg_state[i,2] = 0
    humans_leg_state[i,3] = humans_state[i,0] + leg_dists[i] * jnp.cos(humans_state[i,4] - jnp.pi/2) - (step_length / 2) * jnp.cos(humans_state[i,4])
    humans_leg_state[i,4] = humans_state[i,1] + leg_dists[i] * jnp.sin(humans_state[i,4] - jnp.pi/2) - (step_length / 2) * jnp.sin(humans_state[i,4])
...
@jit
def update_human_legs(human_state, human_leg_state, leg_dists, dt):
    position = human_state[0:2]
    orientation = human_state[4]
    com_left_shifts = jnp.array([
        leg_dists * jnp.cos(orientation + jnp.pi/2),
        leg_dists * jnp.sin(orientation + jnp.pi/2)
    ])
...
@jit
def update_all_humans_legs(humans_state, humans_leg_state, leg_dists, dt):
    return jax.vmap(update_human_legs, in_axes=(0,0,0,None))(humans_state, humans_leg_state, leg_dists, dt)
...
_ = update_all_humans_legs(humans_state, humans_leg_state, leg_dists, dt)
...
for i in range(steps):
    ...
    humans_leg_state = update_all_humans_legs(humans_state, humans_leg_state, leg_dists, dt)
    # Store state
